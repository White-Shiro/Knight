...
FMT_CONSTEXPR auto invoke_parse(parse_context<Char>& ctx) -> const Char* {
  ...
  constexpr bool fmtFormattable =
      std::is_constructible<formatter<mapped_type, Char>>::value;
  if (!fmtFormattable) return ctx.begin();  // Error is reported in the value ctor.
  using formatted_type = conditional_t<fmtFormattable, mapped_type, int>;
